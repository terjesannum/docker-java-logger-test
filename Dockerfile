...
CMD java -Dlogger.sleep=$LOGGER_SLEEP -Dlogger.message.minLength=$LOGGER_MESSAGE_MIN_LENGTH -Dlogger.marker=$LOGGER_MARKER -Dlogger.message.maxLength=$LOGGER_MESSAGE_MAX_LENGTH -Dlogback.configurationFile=$LOGBACK_CONFIG_FILE -Dlog4j.configurationFile=$LOG4J2_CONFIG_FILE -jar /logger-test.jar
